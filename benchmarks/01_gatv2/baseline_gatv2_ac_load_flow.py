...
"""
GATv2 baseline on IEEE 9/14/30/57/118/300 supervised AC load flow.
...
:class:`LocalSumMessagePassingFunction` for :class:`GATv2MessagePassingFunction`
...
This is the Gate-5 reference comparison for Item 1 of
...
def build_gatv2_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a GATv2-equipped GNN matching ready-to-use Tiny / Small.

    Identical to :func:`baseline_gatv2_linearsystem.build_gatv2_gnn`; kept
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_gatv2_ac_load_flow.json"
...
def main() -> int:
    ...
    print("== Gate 5 GATv2 supervised AC load flow on IEEE 9/14/30/57/118/300 ==")
    ...
    print()
    print("== Per-(network, size) summary (GATv2) ==")
