...
"""
Same-hardware perf comparison: LocalSum vs GATv2 message function.
...
``results/perf_gatv2_vs_localsum.json``.
"""
...
def _measure_one(substrate: str) -> dict:
    ...
    for name, builder in (("localsum", _build_localsum), ("gatv2", _build_gatv2)):
    ...
    block["overhead_gatv2_over_localsum"] = {
        "forward": block["gatv2"]["forward_ms_median"] / block["localsum"]["forward_ms_median"],
        "fwd_bwd": block["gatv2"]["fwd_bwd_ms_median"] / block["localsum"]["fwd_bwd_ms_median"],
    }
    block["peak_memory_mb"] = peak_memory_mb()
    print(
        f"  overhead: fwd x{block['overhead_gatv2_over_localsum']['forward']:.2f}"
        f"   fwd+bwd x{block['overhead_gatv2_over_localsum']['fwd_bwd']:.2f}"
...
def main() -> int:
    ...
    print("== Perf: LocalSum vs GATv2 message function ==")
    ...
    out_path = RESULTS_DIR / "perf_gatv2_vs_localsum.json"
