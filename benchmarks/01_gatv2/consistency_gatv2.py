...
"""
Cross-process reproducibility gate for GATv2MessagePassingFunction.

Per the attention-backlog Gate 7 spec ("Bit-identical on IEEE-14 with
fixed seed"), the GATv2 forward pass is exercised on the IEEE-14 AC-LF
context and a SHA-256 hash of the output is written to
``results/consistency_gatv2.json``. Re-running the script must reproduce
...
H2MG context). To isolate the property under test (GATv2 forward
reproducibility) from substrate noise, the first run builds the IEEE-14
context once, pickles it to ``results/consistency_gatv2_context.pkl``,
...
from energnn.model import GATv2MessagePassingFunction
...
def _load_or_build_context():
    ...
    """
    cache_path = RESULTS_DIR / "consistency_gatv2_context.pkl"
...
def main() -> int:
    ...
    payload = {
        ...
        "context_source": "results/consistency_gatv2_context.pkl",
    }
    out_path = RESULTS_DIR / "consistency_gatv2.json"
    out_path.write_text(json.dumps(payload, indent=2))
    print(f"GATv2 output on {NETWORK} sha256 = {h}")
