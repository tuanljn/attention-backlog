...
"""
GlobalAggregation baseline on LinearSystem — apples-to-apples comparison vs LocalSum.
...
for :class:`GlobalAggregationMessagePassingFunction` inside the ``RecurrentCoupler``. The
...
with a side-by-side comparison table for Item 2 (GlobalAggregation) of
...
# is swapped to GlobalAggregation. Larger sizes (Medium / Large / ExtraLarge) deliberately
...
def build_global_aggregation_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a GlobalAggregation-equipped GNN matching the ready-to-use config sizes.

    Identical to ``ReadyRecurrentEquivariantGNN`` except the message function
    is :class:`GlobalAggregationMessagePassingFunction`.
    """
    rngs = nnx.Rngs(seed)
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_global_aggregation_linearsystem.json"
...
def main() -> int:
    ...
    print("== GlobalAggregation baseline on LinearSystem (apples-to-apples vs LocalSum) ==")
    ...
    print()
    print("== Per-size summary (GlobalAggregation) ==")
