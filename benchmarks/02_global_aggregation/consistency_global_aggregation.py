...
"""
Cross-process reproducibility gate for GlobalAggregationMessagePassingFunction.

Per the attention-backlog Gate 7 spec ("Bit-identical on IEEE-14 with
fixed seed"), the GlobalAggregation forward pass is exercised on the IEEE-14 AC-LF
context and a SHA-256 hash of the output is written to
``results/consistency_global_aggregation.json``. Re-running the script must reproduce
...
H2MG context). To isolate the property under test (GlobalAggregation forward
reproducibility) from substrate noise, the first run builds the IEEE-14
context once, pickles it to ``results/consistency_global_aggregation_context.pkl``,
...
from energnn.model import GlobalAggregationMessagePassingFunction
...
def _load_or_build_context():
    ...
    """
    cache_path = RESULTS_DIR / "consistency_global_aggregation_context.pkl"
...
def main() -> int:
    ...
    mf = _build_global_aggregation(structure)
    ...
    payload = {
        ...
        "context_source": "results/consistency_global_aggregation_context.pkl",
    }
    out_path = RESULTS_DIR / "consistency_global_aggregation.json"
    out_path.write_text(json.dumps(payload, indent=2))
    print(f"GlobalAggregation output on {NETWORK} sha256 = {h}")
