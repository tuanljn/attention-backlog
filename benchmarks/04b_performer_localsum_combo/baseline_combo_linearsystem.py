...
"""
Performer baseline on LinearSystem — apples-to-apples comparison vs LocalSum.
...
for :class:`PerformerMessagePassingFunction` inside the ``RecurrentCoupler``. The
...
with a side-by-side comparison table for Item 4 (Performer) of
...
# is swapped to Performer. Larger sizes (Medium / Large / ExtraLarge) deliberately
...
def build_performer_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a Performer-equipped GNN matching the ready-to-use config sizes.

    Identical to ``ReadyRecurrentEquivariantGNN`` except the message function
    is :class:`PerformerMessagePassingFunction`.
    """
    rngs = nnx.Rngs(seed)
    ...
    localsum_mf = LocalSumMessagePassingFunction(
    ...
    performer_mf = PerformerMessagePassingFunction(
        in_graph_structure=in_structure,
        in_array_size=config.latent_dim,
        hidden_sizes=list(config.hidden_sizes),
        activation=nnx.leaky_relu,
    ...
    phi = MLP(
        in_size=2 * config.latent_dim,  # concat of LocalSum + Performer outputs
        hidden_sizes=[],
        activation=nnx.leaky_relu,
        out_size=config.latent_dim,
        use_bias=True,
    ...
    coupler = RecurrentCoupler(
        phi=phi,
        message_functions=[localsum_mf, performer_mf],
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    report = BenchmarkReport(
        ...
        config={
            ...
            "message_function": "PerformerMessagePassingFunction",
    ...
    out_path = RESULTS_DIR / "baseline_combo_linearsystem.json"
...
def main() -> int:
    ...
    print("== Performer baseline on LinearSystem (apples-to-apples vs LocalSum) ==")
    ...
    print()
    print("== Per-size summary (Performer) ==")
