...
"""
Performer baseline on IEEE 9/14/30/57/118/300 supervised AC load flow.
...
:class:`LocalSumMessagePassingFunction` for :class:`PerformerMessagePassingFunction`
...
def build_performer_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a Performer-equipped GNN matching ready-to-use Tiny / Small.

    Identical to :func:`baseline_combo_linearsystem.build_performer_gnn`; kept
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_combo_ac_load_flow.json"
...
def main() -> int:
    ...
    print("== Gate 5 Performer supervised AC load flow on IEEE 9/14/30/57/118/300 ==")
    ...
    print()
    print("== Per-(network, size) summary (Performer) ==")
