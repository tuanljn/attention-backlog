...
"""
VirtualAddressRecurrentCoupler baseline on LinearSystem — apples-to-apples comparison vs LocalSum.
...
for :class:`VirtualAddressRecurrentCoupler` inside the ``RecurrentCoupler``. The
...
with a side-by-side comparison table for Item 4 (VAR-coupler) of
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_var_linearsystem.json"
...
def main() -> int:
    ...
    print("== VirtualAddressRecurrentCoupler baseline on LinearSystem (apples-to-apples vs LocalSum) ==")
    ...
    print()
    print("== Per-size summary (VAR-coupler) ==")
