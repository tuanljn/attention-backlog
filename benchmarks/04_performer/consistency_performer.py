...
"""
Cross-process reproducibility gate for PerformerMessagePassingFunction.

Per Donon's attention-backlog Gate 7 spec ("Bit-identical on IEEE-14 with
fixed seed"), the Performer forward pass is exercised on the IEEE-14 AC-LF
context and a SHA-256 hash of the output is written to
``results/consistency_performer.json``. Re-running the script must reproduce
...
H2MG context). To isolate the property under test (Performer forward
reproducibility) from substrate noise, the first run builds the IEEE-14
context once, pickles it to ``results/consistency_performer_context.pkl``,
...
def _build_performer(structure):
...
def _load_or_build_context():
    ...
    """
    cache_path = RESULTS_DIR / "consistency_performer_context.pkl"
...
def main() -> int:
    ...
    payload = {
        ...
        "context_source": "results/consistency_performer_context.pkl",
    }
    out_path = RESULTS_DIR / "consistency_performer.json"
    out_path.write_text(json.dumps(payload, indent=2))
    print(f"Performer output on {NETWORK} sha256 = {h}")
