...
"""
Same-hardware perf comparison: LocalSum vs Performer message function.
...
- ``linear_system`` -- LinearSystem context, micro-benchmark of the
  ...
  dispatch overhead).
- ``ieee118`` and ``ieee300`` -- the substrates required by Donon's
...
``results/perf_performer_vs_localsum.json``.
"""
...
def _measure_one(substrate: str) -> dict:
    ...
    for name, builder in (("localsum", _build_localsum), ("performer", _build_performer)):
    ...
    block["overhead_performer_over_localsum"] = {
        "forward": block["performer"]["forward_ms_median"] / block["localsum"]["forward_ms_median"],
        "fwd_bwd": block["performer"]["fwd_bwd_ms_median"] / block["localsum"]["fwd_bwd_ms_median"],
    }
    block["peak_memory_mb"] = peak_memory_mb()
    print(
        f"  overhead: fwd x{block['overhead_performer_over_localsum']['forward']:.2f}"
        f"   fwd+bwd x{block['overhead_performer_over_localsum']['fwd_bwd']:.2f}"
...
def main() -> int:
    ...
    print("== Perf: LocalSum vs Performer message function ==")
    ...
    out_path = RESULTS_DIR / "perf_performer_vs_localsum.json"
