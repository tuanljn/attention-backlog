...
"""
MultiHeadQKV baseline on LinearSystem — apples-to-apples comparison vs LocalSum.
...
for :class:`MultiHeadQKVMessagePassingFunction` inside the ``RecurrentCoupler``. The
...
with a side-by-side comparison table for Item 3 (MultiHeadQKV) of
...
# is swapped to MultiHeadQKV. Larger sizes (Medium / Large / ExtraLarge) deliberately
...
def build_multi_head_qkv_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a MultiHeadQKV-equipped GNN matching the ready-to-use config sizes.

    Identical to ``ReadyRecurrentEquivariantGNN`` except the message function
    is :class:`MultiHeadQKVMessagePassingFunction`.
    """
    rngs = nnx.Rngs(seed)
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_multi_head_qkv_linearsystem.json"
...
def main() -> int:
    ...
    print("== MultiHeadQKV baseline on LinearSystem (apples-to-apples vs LocalSum) ==")
    ...
    print()
    print("== Per-size summary (MultiHeadQKV) ==")
