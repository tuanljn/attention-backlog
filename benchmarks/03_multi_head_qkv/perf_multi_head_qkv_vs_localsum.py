...
"""
Same-hardware perf comparison: LocalSum vs MultiHeadQKV message function.
...
``results/perf_multi_head_qkv_vs_localsum.json``.
"""
...
def _measure_one(substrate: str) -> dict:
    ...
    for name, builder in (("localsum", _build_localsum), ("multi_head_qkv", _build_multi_head_qkv)):
    ...
    block["overhead_multi_head_qkv_over_localsum"] = {
        "forward": block["multi_head_qkv"]["forward_ms_median"] / block["localsum"]["forward_ms_median"],
        "fwd_bwd": block["multi_head_qkv"]["fwd_bwd_ms_median"] / block["localsum"]["fwd_bwd_ms_median"],
    }
    block["peak_memory_mb"] = peak_memory_mb()
    print(
        f"  overhead: fwd x{block['overhead_multi_head_qkv_over_localsum']['forward']:.2f}"
        f"   fwd+bwd x{block['overhead_multi_head_qkv_over_localsum']['fwd_bwd']:.2f}"
...
def main() -> int:
    ...
    print("== Perf: LocalSum vs MultiHeadQKV message function ==")
    ...
    out_path = RESULTS_DIR / "perf_multi_head_qkv_vs_localsum.json"
