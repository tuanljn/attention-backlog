...
"""
GlobalAggregation baseline on IEEE 9/14/30/57/118/300 supervised AC load flow.
...
:class:`LocalSumMessagePassingFunction` for :class:`GlobalAggregationMessagePassingFunction`
...
This is the Gate-5 reference comparison for Item 2 of
...
RESULTS_DIR = HERE.parent / "results" / "gap_fill" / "baseline_global_aggregation_capmatched_ac_load_flow"
...
SIZE_CONFIGS = (
    SizeConfig(name="Small", n_breakpoints=20, latent_dim=8, hidden_sizes=(38,), n_steps=10, n_epochs=15),
)


def build_global_aggregation_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a GlobalAggregation-equipped GNN matching ready-to-use Tiny / Small.

    Identical to :func:`baseline_global_aggregation_linearsystem.build_global_aggregation_gnn`; kept
    locally to avoid coupling the two benchmark scripts.
    """
    rngs = nnx.Rngs(seed)
    ...
    message_function = GlobalAggregationMessagePassingFunction(
...
def run_one(network: str, config: SizeConfig, seed: int) -> RunResult:
    ...
    model = build_global_aggregation_gnn(
        config,
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_global_aggregation_ac_load_flow.json"
...
def main() -> int:
    ...
    print("== Gate 5 GlobalAggregation supervised AC load flow on IEEE 9/14/30/57/118/300 ==")
    ...
    print()
    print("== Per-(network, size) summary (GlobalAggregation) ==")
