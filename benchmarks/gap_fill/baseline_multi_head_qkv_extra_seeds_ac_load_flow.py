...
"""
MultiHeadQKV baseline on IEEE 9/14/30/57/118/300 supervised AC load flow.
...
:class:`LocalSumMessagePassingFunction` for :class:`MultiHeadQKVMessagePassingFunction`
...
This is the Gate-5 reference comparison for Item 3 of
...
RESULTS_DIR = HERE.parent / "results" / "gap_fill" / "baseline_multi_head_qkv_extra_seeds_ac_load_flow"
...
def build_multi_head_qkv_gnn(config: SizeConfig, in_structure, out_structure, *, seed: int) -> GNN:
    """Build a MultiHeadQKV-equipped GNN matching ready-to-use Tiny / Small.

    Identical to :func:`baseline_multi_head_qkv_linearsystem.build_multi_head_qkv_gnn`; kept
    ...
    message_function = MultiHeadQKVMessagePassingFunction(
...
def run_one(network: str, config: SizeConfig, seed: int) -> RunResult:
    ...
    model = build_multi_head_qkv_gnn(
        config,
...
def write_partial(all_runs: list[RunResult], status: str) -> Path:
    ...
    out_path = RESULTS_DIR / "baseline_multi_head_qkv_ac_load_flow.json"
...
def main() -> int:
    ...
    print("== Gate 5 MultiHeadQKV supervised AC load flow on IEEE 9/14/30/57/118/300 ==")
    ...
    print()
    print("== Per-(network, size) summary (MultiHeadQKV) ==")
